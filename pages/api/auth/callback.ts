...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const tokenData: DiscordAPIResponse = await fetch(`${process.env.DISCORD_API_ENDPOINT}/oauth2/token`, {
      method: 'POST',
      headers: {
    ...
    const userData = (await fetch(`${process.env.DISCORD_API_ENDPOINT}/users/@me`, {
      headers: {
    ...
    if (userData.email === null) {
      return res
        .status(400)
        .send('No e-mail linked to this account. Make sure everything is correctly set up in your account.');
    }
